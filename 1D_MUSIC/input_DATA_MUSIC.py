...
INPUT = '/eos/user/a/aalvesan/NPLM/Input_2017UL/'      # 2016 or 2017UL !!   path to the MC h5 file to serve as input            

feature_dict    = { 'weight_REF': np.array([]),
                    'weight_DATA': np.array([])}

print ('Reading MC and Data h5 files from ' + str(INPUT) + '\n')

for key in columns_training:
    feature_dict[key] = np.array([])
    key2 = key + '_Data'
    feature_dict[key2] = np.array([])
...
for classname in ML_Classes:                                                                
    ...
    for key in columns_training:
        feature_dict[key] = np.append(feature_dict[key], np.array(f.get(key)))      # e.g. reading MC SumPt
        f.close()
    ...
    for key in columns_training:
        feature_dict[key2] = np.append(feature_dict[key2], np.array(f.get(key)))      # e.g. reading DATA SumPt
        f.close()
...
print ('Before cleaning : REF.shape = ' + str(REF.shape) + ' | W_REF.shape = ' + str(W_REF.shape[0]) + ' | sum(W_REF) = ' + str(N_Bkg) + '\n')
...
W_REF = np.delete(W_REF, event_idx, 0)
REF   = np.delete(REF, event_idx, 0)
W_REF = W_REF * weight_sum_R/np.sum(W_REF) 


print ('After cleaning : REF.shape = ' + str(REF.shape) + ' | W_REF.shape = ' + str(W_REF.shape[0]) + ' | sum(W_REF) = ' + str(N_Bkg) + '\n')
print('W_REF min = ' + str(np.min(W_REF))) 
print('W_REF max = ' + str(np.max(W_REF)) + '\n')
...
print ('Before normalization,: feature.shape = ' + str(feature.shape))
...
print ('After normalization,: feature.shape = ' + str(feature.shape))
print('\nfeature.min   : ' + str(np.min(feature)))
print('feature.max   : ' + str(np.max(feature)))
print ('')
print (feature)
...
plot_training_data(data=DATA, weight_data=weight_DATA, ref=REF, weight_ref=weight_REF,
                   ...
                   ymax_code=ymax_code,save=True, save_path=OUTPUT_PATH, file_name='/plot_REF_DATA_CMS')
...
plt.grid()
plt.savefig(OUTPUT_PATH + '/Training_REF_DATA_CMS.pdf')
plt.close()
